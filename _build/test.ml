...
(** In our OUnit Test Suite, We tested to ensure that the correct state was 
    returned after firing at a coordinate as well as the majority of our functions 
    in the module worked as intended. We tested modules Command, State and Item. 
    Functions in these modules were tested by writing test functions that would 
    compare one object to another, eg. beforeState to an afterState. Our test cases 
    fall under the category of black box testing in which we just worry about 
    the expected output and not the internal aspect of the function. For moduels 
    that wasn't tested in our OUnit test suit was the cpu module and gui module in 
    which was tested manually in the game. Our OUnit Test suit proves the correctness 
    of our system as it ensure the right state is return asfter crucial component of
    the game like firing shot and ensure that players objects (eg. ships, money etc..)
